...
class PerformancePredictor:
    ...
    def predict(
        ...
        for user_idx, x, mask in iter_obj:
            # to gpu
            ...
            # predict
            recon = self.model.predict(user_idx, x)

            # calculate computing cost
            computing_cost = perf_counter() - t0

            # slice
            batch_idx, item_idx = torch.where(mask)
            user_idx_real = user_idx[batch_idx]
            labels = x[batch_idx, item_idx]
            preds = recon[batch_idx, item_idx]

            # to cpu & save
            user_idx_list.extend(user_idx_real.cpu().tolist())
